...
public class EventCommentsActivity extends AbstractOpenScheduleListActivity {
    ...
    // Private methods
    //***************************************
	private class DownloadEventCommentsTask extends AsyncTask<String, Void, List<Comment>> {
		...
		protected List<Comment> doInBackground( String... params ) {
			try {
				return getApplicationContext().getOpenScheduleApi().eventOperations().getEventComments( params[ 0 ] );
			} catch( Exception e ) {
	...
	private void refreshComments() {
		...
		new DownloadEventCommentsTask().execute( selectedEvent );
